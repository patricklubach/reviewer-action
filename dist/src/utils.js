...
exports.reviewersSet = reviewersSet;
...
function validateEvent(eventName) {
    ...
    if (!['pull_request', 'pull_request_review'].includes(eventName)) {
        throw new Error(`Unsupported event type! Supporing: ["pull_request", "pull_request_review"]. Got: ${eventName}`);
    }
...
function reviewersSet(reviewers, pullRequest) {
    ...
    pullRequestRequestedReviewerUsers.forEach((reviewer) => {
        ...
        if (!userReviewers.includes(reviewerName)) {
            core.warning(`Reviewer user ${reviewerName} is not in requested reviewers of PR #${pullRequest.number}`);
            return false;
        }
    });
    ...
    pullRequestRequestedReviewerTeams.forEach((reviewer) => {
        ...
        if (!teamReviewers.includes(reviewerName)) {
            core.warning(`Reviewer team ${reviewerName} is not in requested reviewers of PR #${pullRequest.number}`);
            return false;
        }
    });
    return true;
...
function getCondition(conditionType, pullRequest) {
    core.debug(`Determine condition value based on condition type '${conditionType}'`);
...
function getApprovedReviews(reviews) {
    ...
    try {
        ...
        throw new Error(`Cannot filter reviews for status 'APPROVED'. Details: ${error.message}`);
    }
}
